...
class AnimatedSprite(Blitable, stoppablethread.StoppableThread):
	...
	def run(self):
		...
		while not self.stopped():
			...
			for blitable in self.blitables:
				if blitable.enabled: blitable.blit_on(self.screen)
				else: self.blitables.remove(blitable)
